...
- `src/`: Source files
  ...
  - `help.txt`: Help text
  - `config.sh`: Configuration (version number)
- `dist/`: Build artifacts (generated by build.sh)
- `build.sh`: Build script
- `package.sh`: Packaging script (creates distribution archive)

### Development Workflow

1. Edit files in `src/`
2. Build with `./build.sh`
3. Test with `./dist/ghq-worktree-select`
4. Commit and push (only `src/` is version controlled)
